...
void main() {
  ...
  if (vType < 0.5) {
    ...
    if (noise1 < 0.33) albedo = mix(sakuraLight, sakuraMid, noise1 / 0.33);
    else if (noise1 < 0.66) albedo = mix(sakuraMid, sakuraDeep, (noise1 - 0.33) / 0.33);
    else albedo = mix(sakuraDeep, sakuraRich, (noise1 - 0.66) / 0.34);
    albedo *= 0.65 + min(vLayer / 15.0, 1.0) * 0.35;
  } else if (vType < 2.5) {
    if (noise1 < 0.33) albedo = mix(barkLight, barkMid, noise1 / 0.33);
    else if (noise1 < 0.66) albedo = mix(barkMid, barkDark, (noise1 - 0.33) / 0.33);
    else albedo = mix(barkDark, barkDeep, (noise1 - 0.66) / 0.34);
  } else if (vType < 3.5) {
    albedo = noise1 < 0.3
  ...
  vec3 light = vec3(0.35, 0.38, 0.45)
    + vec3(1.15, 1.05, 0.95) * ndSun
    + vec3(0.85, 0.90, 0.95) * ndUp * 0.22;
  ...
  gl_FragColor = vec4(acesFilm(mix(shaded, scanColor, scanAmount)), 1.0);
}
`;

type Props = { matrix: QRMatrix; scanMode: boolean };
...
function buildGeometry(matrix: QRMatrix) {
  ...
  const centerAttribute = new THREE.Float32BufferAttribute(positions, 3);
  geometry.setAttribute('position', centerAttribute);
  geometry.setAttribute('aCenter', centerAttribute.clone());
